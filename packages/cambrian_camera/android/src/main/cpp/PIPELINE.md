...
Peak: 3 + 4 = 7 threads during saturation.
...
`Consumer::mu` only long enough to swap the `pending` shared_ptr. If the consumer
is slow, the previous frame is dropped and replaced with the latest — latest-frame
...
processingLoop()  [processing thread]
    ...
    │     Output: cv::Mat bgr  CV_8UC3  4160×3120  (37.1 MB)   ~15ms
    │
    ...
    │         → join 4 threads
    │         → processed: new 37.1 MB cv::Mat      ~25ms (debug -O2)
    │         modified = true
    │     else:
...
### AE target FPS range
`CONTROL_AE_TARGET_FPS_RANGE` is set to the highest available min-fps range
(preferring `[30, 30]`). Without it, Camera2 AE can ramp exposure above 1/30 s —
at 70 ms exposure the sensor physically cannot deliver more than ~14 fps regardless
of pipeline speed.
...
when all 4 slots are occupied — processing slowdowns degrade frame rate gracefully.
...
The raw preview consumer is outside `consumers_` so it receives pre-processing BGR
independently. `shutdownConsumers()` (teardown of user sinks) does not affect it.

### 1-slot mailbox (latest-frame semantics)
Each consumer holds at most one pending frame. Slow consumers drop frames
independently — one slow consumer cannot block others.
...
Android big cores. GPU path planned for a future session.
...
```
perf [240] fps=23.5  total=42.5ms  pop=0.2ms  yuv=14.9ms  sat=24.9ms  pub=2.6ms
```
...
`total ≈ pop + yuv + sat + pub`. A large `pop` means the pipeline is idle
waiting for the camera (camera is the bottleneck). A near-zero `pop` means
the pipeline is the bottleneck (processing can't keep up with camera delivery).
