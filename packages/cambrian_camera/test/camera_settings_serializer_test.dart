...
void main() {
  ...
  group('ProcessingParams', () {
    ...
    test('toCam passes contrast through unchanged', () {
      // Dart and engine share the same convention ([-1,1], 0.0 = identity), so
      // contrast is forwarded verbatim — no offset translation.
      expect(ProcessingParams(contrast: 0.0).toCam().contrast, 0.0);
      expect(ProcessingParams(contrast: 0.5).toCam().contrast, 0.5);
      expect(ProcessingParams(contrast: -1.0).toCam().contrast, -1.0);
    });
