...
2026-05-22 [trackertex] coordinator — `captureNaturalPicture` re-implemented as an `AVCapturePhotoOutput` one-shot (ISP hardware-processed) → `MetalPipeline.gradeOneShot` (live crop + Pass-2 grade) → **TIFF** cropped to the active region. Now identical to `captureImage` except for its source (ISP one-shot vs live video frame): same shared `AVCaptureDevice` settings (ISO/exposure/WB/focus) and same live `ColorUniform` grade. **Reverses D-2P-10** and supersedes `docs/superpowers/specs/2026-05-15-capture-natural-picture-design.md`; D-2P-12's "retains D-2P-10" note no longer applies. Pieces: `AVCapturePhotoOutput` attached at `open()` (`CameraSession.configure`, `canAddOutput`-guarded, rotation matched ADR-17); `StillPhotoCapture` (settings: 420f, flash off, `.balanced`, no high-res/RAW/HDR override — honors device locks; delegate→continuation bridge, capture fired on `sessionQueue` per ADR-07); `MetalPipeline.gradeOneShot(pixelBuffer:)` runs on the engine actor (NOT sessionQueue) reusing the live PSOs/uniforms, with a 1:1 crop mapping guarded by an input-dims==captureSize check (`MetalError.unsupportedFormat`). Pause contract changed (R6): errors cleanly (`EngineError.capture(.bufferUnavailable)`) when the session isn't running — no last-frame fallback. `captureImage` untouched. Status: code-complete, unit-verified on physical iPad (206/206 suite green, 0 warnings; `gradeOneShot` grade+dimension-guard tests, settings test, not-open guard test). **HITL pending** (ISP visual quality / grade match / crop framing / saved-TIFF orientation / pause-error / capture-returns) — the #1 on-device risk is the photo buffer dims defaulting larger than `captureSize` (would trip the dimension guard → clean throw); prepared fix is pinning `photoOutput.maxPhotoDimensions` to the active format's video dims. Per `docs/superpowers/specs/2026-05-22-isp-natural-capture-design.md` + `docs/superpowers/plans/2026-05-22-isp-natural-capture.md`.
